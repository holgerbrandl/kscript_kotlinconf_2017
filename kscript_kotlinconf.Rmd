...
### Fast to write. Impossible to maintain. Not even portable. Does not scale. Unteachable black art.
...
* No built-in way to indicate a specific version for a project
...
Would work using `kotlinc -script` as interpreter as well, but would be painfully slow because of recompilation on each invocation
...
1. **Interactivity**: Continue jupyter-kernel support  (interactive usage, but also report rendering using Kotlin scripts)
