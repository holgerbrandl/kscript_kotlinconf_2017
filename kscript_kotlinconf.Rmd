...
output:
  xaringan::moon_reader:
    lib_dir: libs
    css: ["default", "fonts", "my-theme.css"]
...
### 2. Rapid Prototyping: Can we quickly run an analysis?

### 3. Scalable: Can we scale simple protocols into more complex systems?
...
likes
* Static typing providing guidance and context
* Type-time checking of code thanks to IDEs
* Deployment of an product
* Java dependency trees rock!

--

dislikes
* Prototyping happend in the debugger
...
### 2. Rapid Prototyping: Can we quickly do setup an analysis?            ![](fail.png)

### 3. Scalable: Can we scale simple protocols into more complex systems? ![](check.png)
...
---
# Python and R for data-driven sciense?
...
.footnote[[Why I hate virtualenv and pip](https://pythonrants.wordpress.com/2013/12/06/why-i-hate-virtualenv-and-pip/)]
...
> But it's faster and way shorter to do it bash! (colleagues)

Do _magical_ thing in the terminal
```bash
removeMultiMappers(){
...
Not even truly portable. Does not scale, Unteachable black art.
...
---
# Stick to the basics to allow for proper tool integration?


Easy to to polyglot programming.

Enable integration

The linux shell just rocks

```bash
zcat seq_dasta.fastq.gz  | \
awk '{sub(/[ \t]*$/, "");print}' | \
python -c '
...
for record in SeqIO.parse(sys.stdin, "fastq"):
  if (record.description.split(" ")[1].startswith("1:")) : print record.description
' | \
gzip -c > processed_data.gz
```


`R` and `python` integrate nicely with `bash` *without* having any additional scripts floating around
```bash
cat iris.csv | \
    Rscript --vanilla -e 'read.csv(file("stdin")) %>% dplyr::filter(Species=="setosa") %>% write.table(stdout()) ' | \
    cut -f-1,3 > setosa.txt
```

Often best way if specialized tools get involved.

Pick the best from all worlds? Sure, but in a solid way avoid ugle dependencies.

???

Especiall in science not everythone is working with services all day, but we stick existing cutting-edge tools to build workflows.

rocks ... screen, coloring, pipes.
...
background-image: url(images/927721c9.png)
background-position: center
background-repeat: no-repeat
background-size: 110%

# How to give it to the community? Dockerize it!?


---
background-image: url(images/927721c9_white.jpg)
background-position: center
background-size: 110%
...
## Sucessfully published! ![](fail.png)
...
## Doing just docker-support since then. ![](fail.png)
...
---
# State of art in data-driven science

Reproducible, Dependencies & Scalbilty: Java

Protyping & Community: **R & Python**


What's needed for scientic software development?
...
.image-50[![](images/7d17d6d4.png)]


.footnote[From german _Eierlegende Wollmichau_]
...
--
# End 2015: First `kotlin` encounter!
...
* all in a jvm context with nice dependencies


**TODO** simley here!

Ready for reproducible analysis, Rapid Prototyping, proper dependeny model, Community
...
* But how to pull in dependencies? `kotlin -cp ...` seemed too tedious
* Also since eval means recompile, it felt a bit slow
* How to keep track of required JVM options
