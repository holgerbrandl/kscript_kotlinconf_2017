...
Already a viable alternative to write scripts, but not already better than `kotlinc -script`
...
> Script sources are (often) not enough to _... write self-contained mini-applications with Kotlin._ (from kscript GH-README)
...
---
# Ditch awk by using `kscript`s support API

`kscript` can be used as a speedier and flexible substitute for built-in terminal tools such as `awk` or `sed`


`awk`-mode: Scripts starting with `lines` enable a support-api for line-based text-processing.
...
```
Bootstrap interactive shell. Example: `CountRecords.kts`
...
#### Not many. Don't get infected by _featureritis_!
...
* Revise annotation-driven script configuration once Scripting API (KEEP75 + keplin) supports it
...
---
## What's still missing in kotlin to become the next big thing for  in data-science?

1. **Interactivy**: Continue jupyter-kernel support  (interactive usage, but also report rendering using kotlin scripts)
2. Provide a cut-down version IJ for Kotlin. Newbies are overwhelmed with current builds. Would also help for teaching
3. Fix the REPL to become truly interactive
4. Better visualization (dplyr, d3)
5. More libraries. Just wrap existing JVM libraries!
...
### Languages for data-science must allow to share code between prototypes and products, should quick to get started with, and should allow for rapid prototyping
